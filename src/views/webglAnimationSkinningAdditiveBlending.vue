...
function createPanel() {
  ...
  settings = {
    ...
    动画5: function () {
      ...
      if (!meshAnimation) return;
      const { clipActions } = meshAnimation;
      meshAnimation.selectClipAction(clipActions[4]);
    },
    动画6: function () {
      const { meshAnimation } = model;
      if (!meshAnimation) return;
      const { clipActions } = meshAnimation;
      meshAnimation.selectClipAction(clipActions[5]);
    },
    动画7: function () {
      const { meshAnimation } = model;
      if (!meshAnimation) return;
      const { clipActions } = meshAnimation;
      meshAnimation.selectClipAction(clipActions[6]);
    },
  ...
  folder4.add(settings, "动画5");
  folder4.add(settings, "动画6");
  folder4.add(settings, "动画7");
}
